...
import type { LoginProfile, PaginatedResponse, SessionUser, TokenPair } from '../types';
 ...
 */
export async function login(username: string, password: string, profile: LoginProfile) {
  const response = await api.post<TokenPair>('/auth/token/', { username, password, profile });
