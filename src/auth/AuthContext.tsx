...
import type { LoginProfile, SessionUser, UserProfile } from '../types';

type AuthContextValue = {
  user: SessionUser | null;
  loading: boolean;
  isAuthenticated: boolean;
  login: (username: string, password: string, profile: LoginProfile) => Promise<SessionUser>;
  logout: () => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const handleLogin = useCallback(async (username: string, password: string, profile: LoginProfile) => {
    await apiLogin(username, password, profile);
    const session = await fetchMe();
    setUser(session);
    return session;
  }, []);
